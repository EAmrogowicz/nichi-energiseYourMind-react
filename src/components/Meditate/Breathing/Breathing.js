...
export default function MeditateBreath() {
  ...
  //--PW to solve the React warning "Function makes the dependencies of useEffect Hook change on every render":
  //-- (1) cannot move playPause() into useEffect as it is needed inside the JSX when playPause button is clicked
  //-- (2) so instead wrap the playPause() function definition in a useCallback hook which will hopefully return a memoized function whose reference will only change if something in the hook's dependency array changes. 
      ...
      {!isEnded ? (
        <div className="meditateContainer">
          ...
          <div className="animeContainer">
            <div
              className={`blobbly-blob ${isActive ? "blobbly-blob-swell" : ""}`}
            ></div>
          </div>
          <Stack>
            <Box
